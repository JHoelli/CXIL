...
def augment(image, digit, randomize=False, mult=25, all_digits=range(10)):
  #TODO 
  '''
  image x have a 4x4 gray  swatch in randomly chosen conrner. 
  Train Set: Function of digit y 
  train Set : Train set with random swatch
  '''
  if randomize:
  ...
  for i in dir1:
    for j in dir2:
      img[i][j][0] = 255 - mult*digit
      img[i][j][1] = 255 - mult*digit
      img[i][j][2] = 255 - mult*digit
      expl[i][j] = 1
...
def generate_dataset(N=20000, cachefile='./data/decoy-toy-colors.npz'):
  ...
  for image, digit in zip(X, Y):
    x, e = augment(image.reshape(5,5,3), digit, all_digits=list(np.unique(Y)))
    X_new.append(x)
    E.append(e)
  for image, digit in zip(Xt, Yt):
    xt, e = augment(image.reshape(5,5,3), digit, all_digits=list(np.unique(Yt)), randomize=True)
    Xt_new.append(xt)
    Et.append(e)
  ...
  data = (
      np.array(X_new),
      np.array(Xt_new), train_y, test_y,E,Et)
...
if __name__ == '__main__':
  import pdb
  X, Xt, y, yt,E,Et = generate_dataset()
  #pdb.set_trace()
  #pass