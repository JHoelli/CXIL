...
def load_physiology_data(basepath='/XIL/Data/data/PhysioNetChallenge2020_Training_CPSC/Training_WFDB'):
    '''
    ...
    if os.path.exists(basepath):
        ...
        os.mkdir(basepath)
        filename=wget.downlaod('https://physionet.org/files/ptb-xl/1.0.1/',out=f'{basepath}')
    ...
    for leads in leads_of_interest:
        ...
        inputs,outputs,all_labels = generate_inputs_and_outputs(files,basepath,leads,samples_to_take,trial)
        outputs = encode_outputs(all_labels,outputs)
        makepath_and_save_data(inputs,outputs,leads,trial=trial)

def generate_inputs_and_outputs(files,basepath,leads_of_interest,samples_to_take,trial):
...
def makepath_and_save_data(basepath,inputs,outputs,leads_of_interest,trial=''):
