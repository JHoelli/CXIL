...
if not os.path.isdir(f'./Results/{data_name}/caipi'):
    os.mkdir(f'./Results/{data_name}/caipi')
if not os.path.isdir(f'./Results/{data_name}/caipi/{parameter}'):
    os.mkdir(f'./Results/{data_name}/caipi/{parameter}')
if not os.path.isdir(f'./Results/{data_name}/caipi/{parameter}/{str_model}'):
    os.mkdir(f'./Results/{data_name}/caipi/{parameter}/{str_model}')


if not rerun_all: 
    print('RERUN ONLY NECESSAEY')
    # Try to load data 
    if os.path.exists(f'./Results/{data_name}/caipi/{parameter}/{str_model}/classification_metrics_{seed}.csv'):
        print('FGet Classifciation ')
        full_classification_df=pd.read_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/classification_metrics_{seed}.csv')  
        full_classification_df.drop(labels=['Unnamed: 0'],axis=1,inplace=True)
        full_interpretability_df= pd.read_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/interpretability_metrics_{seed}.csv')
        ...
        full_interpretability_df.drop(labels=['Unnamed: 0'],axis=1,inplace=True)
    #if os.path.exists(f'./Results/{data_name}/caipi/{parameter}/{str_model}/interpretability_metrics_{seed}.csv'):
        #full_interpretability_df= pd.read_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/interpretability_metrics_{seed}.csv')
        #full_interpretability_df.drop(labels=['Unnamed: 0'],axis=1,inplace=True)
    #elif os.path.exists(f'./Results/{data_name}/classification_metrics_{seed}.csv'):
        #full_classification_df=pd.read_csv(f'./Results/{data_name}/classification_metrics_{seed}.csv')  
...
#sys.exit(1)
'''Load Data '''
if 'Sim' in dataset or 'ime' in dataset:#os.path.isfile(dataset):
    print('----TimeSeries----')
...
if X_train is not None:
    ...
    X_with_finetuning=X#[0:10]
    y_with_finetuning=y#[0:10]
...
##############################################################################
#'''TODO DELETE No Training & CAIPI - iterativly'''
#model_empty=model_our
#m_wrap_empty=mod_wrapper(model_empty)

#trans= Transformer.DataBasedTransformer(X_train, y_train,m_wrap_empty.predict)
#trans= trans_function(m_wrap_empty.predict,num_samples=1)
...
#cai=caipi(model_empty,  learn,predict_func=m_wrap_empty.predict,transformer=trans, evaluate_data= (X_test,y_test),silent=1,simulation_logic=simulation)
#acc,f1, precision, recall=cai.iterate(X_with_finetuning)
##############################################
#import sys
#sys.exit(1)
...
#clas,interpret= evaluation(model_our, InputXGradient, (X_test,y_test),f'./Results/{data_name}/caipi/{parameter}','iter half',save=True,ground_truth=ground_truth)
...
'''
if rerun_all or os.path.exists(f'./Results/{data_name}/model_empty_{seed}'):
    '''No Training'''
...
if rerun_all or not os.path.exists(f'./Results/{data_name}/empty_trained_{seed}'):
    '''No Training & CAIPI - iterativly'''
    ...
    cai=caipi(model_empty,  learn,transformer=trans, evaluate_data= (X_test,y_test),silent=1,simulation_logic=simulation)
    acc,f1, precision, recall, loss_calc_train, loss_calc_test=cai.iterate(X_with_finetuning,y_with_finetuning, taskid=None)
    h=f'emptytrained_{seed}'
    pd.DataFrame(acc).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_acc_{h}.csv')
    pd.DataFrame(f1).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_f1_{h}.csv')
    pd.DataFrame(precision).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_precision_{h}.csv')
    pd.DataFrame(recall).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_recall_{h}.csv')
    pd.DataFrame( loss_calc_train).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/loss_train_{h}.csv')
    pd.DataFrame( loss_calc_test).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/loss_test_{h}.csv')

    clas,interpret= evaluation(cai.model, InputXGradient, (X_test,y_test),f'./Results/{data_name}/caipi/{parameter}/{str_model}',f'empty_trained_{seed}',save=True,ground_truth=ground_truth)

    full_classification_df=pd.concat([full_classification_df,clas], ignore_index=True)
    full_interpretability_df=pd.concat([full_interpretability_df,interpret], ignore_index=True)
    full_classification_df.to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/classification_metrics_{seed}.csv')
    full_interpretability_df.to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/interpretability_metrics_{seed}.csv')

'''
if rerun_all or not os.path.exists(f'./Results/{data_name}/caipi/{parameter}/{str_model}/model_half_trained'):
    if 'CNN' in str(model):
    ...
    trans= trans_function(model_half_n,num_samples=1)
    #exp=explainer.explain_instance
    ...
    cai=caipi(model_half_n,  learn,transformer=trans, evaluate_data= (X_test,y_test),silent=1,simulation_logic=simulation)
    acc,f1, precision, recall=cai.iterate(X_train, taskid=None)
    h='modelhalftrained'
    pd.DataFrame(acc).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_acc_{h}.csv')
    pd.DataFrame(f1).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_f1_{h}.csv')
    pd.DataFrame(precision).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_precision_{h}.csv')
    ...
    clas,interpret= evaluation(cai.model, InputXGradient, (X_test,y_test),f'./Results/{data_name}/caipi/{parameter}/{str_model}','model_half_trained',save=True,ground_truth=ground_truth)


    full_classification_df=pd.concat([full_classification_df,clas], ignore_index=True)
    full_classification_df.to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/classification_metrics.csv')
    full_interpretability_df=pd.concat([full_interpretability_df,interpret], ignore_index=True)
    full_interpretability_df.to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/interpretability_metrics.csv')
'''

if rerun_all or not os.path.exists(f'./Results/{data_name}/caipi/{parameter}/{str_model}/model_full_trained_{seed}'):
    ...
    #exp=explainer.explain_instance
    trans=trans_function(model_full_n,num_samples=1)
    learn = LearnerStep.Basic(model_full_n)#LearnerStep.BasicLearner(model_empty)
    cai=caipi(model_full_n, learn,transformer=trans, evaluate_data= (X_test,y_test),silent=1,simulation_logic=simulation)
    ...
    pd.DataFrame( loss_calc_test).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/loss_test_{h}.csv')
    #TODO save plots
    clas,interpret= evaluation(cai.model, InputXGradient, (X_test,y_test),f'./Results/{data_name}/caipi/{parameter}/{str_model}',f'model_full_trained_{seed}',save=True,ground_truth=ground_truth)
...
if rerun_all or not os.path.exists(f'./Results/{data_name}/caipi/{parameter}/{str_model}/empty_trained_replay_{seed}'):
    ...
    #    simulation.update_meta(meta_with_finetuning)
    learn = LearnerStep.Basic(model_empty)#LearnerStep.BasicLearner(model_empty)
    learn_replay=Replay.ReplayStrategyLearner(learn, num_classes=n_classes,mem_size = 100,batch_size= 32)
    #simulation= timeseries(dataset,meta_with_finetuning).simulate
    cai=caipi(model_empty,  learn_replay,transformer=trans, evaluate_data= (X_test,y_test),silent=1,simulation_logic=simulation)

    acc,f1, precision, recall, loss_calc_train, loss_calc_test=cai.iterate(X_with_finetuning,y_with_finetuning, taskid=None)
    h=f'emptytrainedreplay_{seed}'
    pd.DataFrame(acc).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_acc_{h}.csv')
    pd.DataFrame(f1).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_f1_{h}.csv')
    pd.DataFrame(precision).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_precision_{h}.csv')
    pd.DataFrame(recall).to_csv(f'./Results/{data_name}/caipi/{parameter}/{str_model}/running_recall_{h}.csv')
    ...
    clas,interpret= evaluation(cai.model, InputXGradient, (X_test,y_test),f'./Results/{data_name}/caipi/{parameter}/{str_model}',f'empty_trained_replay_{seed}',save=True,ground_truth=ground_truth)
...
'''
if rerun_all or not os.path.exists(f'./Results/{data_name}/caipi/{parameter}/{str_model}/model_half_trained_replay'):
    ...
    #if 'meta_train' in locals():
    #    simulation.update_meta(meta_train)
    learn_replay=Replay.ReplayStrategyLearner(learn, num_classes=n_classes,mem_size = 500,batch_size= 32)
    cai=caipi(model_half_n,  learn,transformer=trans, evaluate_data= (X_test,y_test),silent=1,simulation_logic=simulation)
    acc,f1, precision, recall=cai.iterate(X_train, taskid=None)
    ...
    clas,interpret= evaluation(cai.model, InputXGradient, (X_test,y_test),f'./Results/{data_name}/caipi/{parameter}/{str_model}','model_half_trained_replay',save=True,ground_truth=ground_truth)
...
'''
if rerun_all or not os.path.exists(f'./Results/{data_name}/caipi/{parameter}/{str_model}/model_full_trained_replay_{seed}'):
    '''Full Training & CAIPI'''
    if 'CNN' in str(model):
        model_full_n = model(num_classes=n_classes) 
    else:
    ...
    learn = LearnerStep.Basic(model_full_n)#LearnerStep.BasicLearner(model_empty)
    learn_replay=Replay.ReplayStrategyLearner(learn, num_classes=n_classes,mem_size = 100,batch_size= 32)
    cai=caipi(model_full_n, learn_replay,transformer=trans, evaluate_data= (X_test,y_test),silent=1,simulation_logic=simulation)
    #torch.save(cai.model.state_dict(), f'./Results/{data_name}/{parameter}/model_full_trained')
    acc,f1, precision, recall, loss_calc_train, loss_calc_test =cai.iterate(X_with_finetuning,y_with_finetuning, taskid=None)
    ...
    clas,interpret= evaluation(cai.model, InputXGradient, (X_test,y_test),f'./Results/{data_name}/caipi/{parameter}/{str_model}',f'model_full_trained_replay_{seed}',save=True,ground_truth=ground_truth)
